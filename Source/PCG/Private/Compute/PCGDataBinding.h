...
public:
	...
	/** Get the table of attributes present in the compute graph element input data. */
	const TMap<FName, FPCGKernelAttributeIDAndType>& GetInputAttributeTable() const { return InputAttributeTable; }
...
private:
	...
	void BuildInputAttributeTable(const FPCGDataCollection& InComputeGraphElementInputData);
...
protected:
	/** Attributes read/written/created by kernels. */
	UPROPERTY()
	TMap<FName /* Attribute name */, FPCGKernelAttributeIDAndType> GlobalAttributeLookupTable;

	/** Attributes present across all input data. */
	UPROPERTY()
	TMap<FName /* Attribute name */, FPCGKernelAttributeIDAndType> InputAttributeTable;
