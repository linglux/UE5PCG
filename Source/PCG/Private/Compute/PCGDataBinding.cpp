...
void UPCGDataBinding::InitializeInputData(const FPCGDataCollection& InComputeGraphElementInputData)
{
	...
	BuildInputAttributeTable(InComputeGraphElementInputData);
}

void UPCGDataBinding::BuildInputAttributeTable(const FPCGDataCollection& InComputeGraphElementInputData)
{
	const UEnum* PCGKernelAttributeTypeEnum = StaticEnum<EPCGKernelAttributeType>();
	check(PCGKernelAttributeTypeEnum);

	const FPCGDataCollectionDesc Desc = FPCGDataCollectionDesc::BuildFromDataCollection(InComputeGraphElementInputData, GetAttributeLookupTable(), GetStringTable());
	for (const FPCGDataDesc& DataDesc : Desc.DataDescs)
	{
		for (const FPCGKernelAttributeDesc& AttrDesc : DataDesc.AttributeDescs)
		{
			// Skip non-attributes like point properties.
			if (AttrDesc.Name == NAME_None)
			{
				continue;
			}

			if (FPCGKernelAttributeIDAndType* AttrIDAndType = InputAttributeTable.Find(AttrDesc.Name))
			{
				if (AttrIDAndType->Type != AttrDesc.Type)
				{
					UE_LOG(LogPCG, Warning, TEXT("Encountered attribute '%s' multiple times in input data with different types (%s vs %s). Attributes currently must have a single type in compute graphs."),
						*AttrDesc.Name.ToString(),
						*PCGKernelAttributeTypeEnum->GetNameStringByValue(static_cast<int64>(AttrDesc.Type)),
						*PCGKernelAttributeTypeEnum->GetNameStringByValue(static_cast<int64>(AttrIDAndType->Type)));
				}
			}
			else
			{
				const FPCGKernelAttributeIDAndType IDAndType(AttrDesc.Index, AttrDesc.Type);
				InputAttributeTable.Add(AttrDesc.Name, IDAndType);
			}
		}
	}
}
